...
### 18.11 桃曲坡基础数据探查（2026-09-11 19:30，后台 API 全量普查）

**背景**：桃曲坡水利项目基础台账迁入测试项目 10462。对 244 个模型逐个 `getModelDataMeta`（计数+抽样）普查：**15 个有数据、222 个空、7 个报错**（网关任务族平台模型，网关侧 SQL 问题）。

**桃曲坡数据画像**（均已验证可查询）：
- **水库 3 座**（水库基础模型）：桃曲坡水库（工程规模 3、坝址控制流域面积 830km²、总库容 5720 万 m³、灌溉/防洪/城镇供水）、尚书水库（37km²/212.5 万 m³）、红星水库（236.9km²/799 万 m³）。
- **河流 10 条**（河流基础模型）：渭河（818km/137626km²）、泾河（455km/45363km²）、沮河、漆水河、马栏河、石川河、温泉河、顺阳河、赵氏河、三水河，含级别/长度/流域面积/流经地区。
- **灌区渠系 60+ 条**（设施基础模型 75 行，编码前缀 TQQX=桃曲坡灌区）：干渠 9 条（东干渠/南干渠/南干渠(2)/尚书干渠/广惠渠/民联干渠/西干渠/金定渠/高干渠，TQQX-G0xx）+ 支渠 47 条（TQQX-Zxxx）+ 溢洪道1/引水洞1。
- 设备：水泵 4 台（28 测点时序在采）、无人机（大疆 Mavic4 Pro）、无人船（潜鲟F1PRO）；组织：用户 201/职工 200。

**缺口（AGP 侧）**：① 水利监测时序未接入——水位计/库水位计/雨量计/流量计/渗压计/水文监测站模型 0 行、0 测点绑定，"当前水位/降雨"类问题暂不可答；② 设施族模型无「内部编码」列（用设施编码），`getObjetTags` 对其报 SQL 错，对象级测点语义查询目前仅设备族（水泵模型）可用。

**E2E 逼出的修正（真实浏览器会话验证）**：
- `query_model`/`model_attributes` 的 model_name 用 list_models 的**英文名 class_name（wt_116_xxx）会报"模型不存在"**，必须用中文别名 class_alias——此前工具描述写着"模型英文名"属误导，已改正（query_model/model_attributes/query_model_segment 三处 + persona 第 2 条通用规则）。
- where_str 属性名各模型不同（设备族"名称"、设施族"设施名称"），写错报 Unknown column；agent 实测中自行经 model_attributes 查出正确属性恢复。persona 第 3 条已补"以 model_attributes 返回的中文属性为准"。
- persona 新增第 3 条项目概况段（桃曲坡：水库/河流/渠系入口模型 + 旗舰数字 + 时序缺口 + 设施族 object_tags 不可用），"有哪些水库/干渠/河流"类名单问题引导为 query_model 取数而非记忆枚举。

**复验**（新会话，deepseek-v4-flash-0731，19:47）："桃曲坡灌区有哪些干渠？桃曲坡水库的总库容和坝址控制流域面积是多少？" → `list_models(设施/水库)` → `model_attributes` ×2 → `query_model` ×3，25 秒 6 步，回答 9 条干渠清单（TQQX-G0xx）+ 47 条支渠说明 + 桃曲坡水库 5720 万 m³/830km²/灌溉防洪城镇供水，并主动交叉核对不含"干渠"字样的广惠渠/金定渠、注明无实时监测时序。
