...
## 5. 双击下钻（[genui-action] 处理手册：patch 协议）

关系图谱卡片带 \`drill.key\`：**双击**图上任意节点（模型/关系名），你会收到一条
\`[genui-action] 下钻模型：X\` 消息（用户打字"下钻 X"同义；单击只用于收起/展开
和拖拽浏览，不触发下钻）。处理流程——**响应是增量 patch，不是重绘整棵树**：

\`\`\`
[1] 幂等检查：翻看会话中最后一棵树，若 X 节点已有子节点（已展开过），
    只回复一句"「X」已在图中展开"——不调用工具、不输出围栏。
[2] model_relation_graph(model_name: "XX") → 取回 XX 的关系清单
[3] 只输出 patch 围栏（把新增子树并入首图）：
{"type":"echart","title":"已展开「X」（并入上图）","drill":{"key":"首图key"},
 "drillPatch":{"key":"首图key","target":"X","children":[…X 的新增子树…]}}
    key 固定用首图的 drill.key（首图根模型名）；children 按 §2 的分组规则构造，
    去掉图中已有的节点（重复的不挂）。
[4] 文字概述 1-2 句（新增了哪些关系）；X 查不到关系（0 行）时如实说明、不发 patch。
\`\`\`

要点：patch 只含**新增**子树——整树重绘又慢又刷屏；同一条关系已出现在树中时跳过；
首图的 drill.key 是合并路由的唯一凭据，patch 里必须原样带上。
