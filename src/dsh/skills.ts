...
## 5. 点击下钻（[genui-action] 处理手册：patch 协议）

关系图谱卡片带 \`drill.key\` + \`actionTemplate:"下钻模型：{name}"\`：用户点击图上
任意节点，你会收到一条 \`[genui-action] 下钻模型：X\` 消息（用户打字"下钻 X"同义）。
处理流程——**响应是增量 patch，不是重绘整棵树**：
